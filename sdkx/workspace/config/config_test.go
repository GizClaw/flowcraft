...
func TestBuildRejectsUnknownFieldsDriverAndNilAsValidation(t *testing.T) {
	...
	if err := builder.RegisterFactory(config.DriverMemory, func(context.Context, *yamlv3.Node) (config.Resource, error) {
	...
	nilResourceCloses := 0
	if err := builder.RegisterFactory("nil-resource", func(context.Context, *yamlv3.Node) (config.Resource, error) {
...
func TestCustomFactoryAndImmutableRegistry(t *testing.T) {
	...
	if err := builder.RegisterFactory("custom", func(_ context.Context, settings *yamlv3.Node) (config.Resource, error) {
		type customSettings struct {
			Root string `yaml:"root"`
		}
		s, err := config.DecodeSettings[customSettings](settings)
		if err != nil {
...
func TestFactoryErrorsAreValidation(t *testing.T) {
	builder := config.NewBuilder(config.Deps{})
	if err := builder.RegisterFactory("broken", func(context.Context, *yamlv3.Node) (config.Resource, error) {
...
func TestFactoryPreservesClassifiedErrors(t *testing.T) {
	builder := config.NewBuilder(config.Deps{})
	if err := builder.RegisterFactory("unavailable", func(context.Context, *yamlv3.Node) (config.Resource, error) {
...
func TestFactoryClassifiesContextCancellation(t *testing.T) {
	builder := config.NewBuilder(config.Deps{})
	if err := builder.RegisterFactory("cancelled", func(context.Context, *yamlv3.Node) (config.Resource, error) {
...
func TestRegistryOwnsFactoryWorkspaceLifecycle(t *testing.T) {
	for _, failAfterBuild := range []bool{false, true} {
		t.Run(map[bool]string{false: "registry close", true: "partial failure"}[failAfterBuild], func(t *testing.T) {
			...
			builder := config.NewBuilder(config.Deps{})
			if err := builder.RegisterFactory("closeable", func(context.Context, *yamlv3.Node) (config.Resource, error) {
