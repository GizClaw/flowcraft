...
func TestRoundTripHistoryPersistsOnlyPublishedAssistantMessagesWithName(t *testing.T) {
	ws, err := workspace.NewLocalWorkspace(t.TempDir())
	if err != nil {
		t.Fatalf("NewLocalWorkspace: %v", err)
	}
	llm := &capturingLLM{replies: []string{
		"case_action: kind=inspect_gramophone; target=gramophone",
		"你检查留声机后发现，它被设置成了单曲循环模式。",
	}}
	app := newTestClaw(t, ws, llm, func(cfg *Config) {
		publishFalse := false
		publishTrue := true
		...
		cfg.Agent.Publisher.Nodes = map[string]NodePublishConfig{
			"format_player_action": {Publish: &publishFalse},
			"generate_game_master": {Publish: &publishTrue},
		}
		cfg.Agent.Graph = graph.GraphDefinition{
			Name:  "murder-history",
			Entry: "format_player_action",
			...
			Edges: []graph.EdgeDefinition{
				{From: "format_player_action", To: "generate_game_master"},
				{From: "generate_game_master", To: graph.END},
			},
		}
	})
	defer app.Close()

	resp, err := app.RoundTrip(Request{Text: "我想看看留声机"})
	if err != nil {
		t.Fatalf("RoundTrip: %v", err)
	}
	var streamed strings.Builder
	for {
		...
		case EventToken:
			streamed.WriteString(ev.Content)
			if ev.NodeID != "generate_game_master" {
				t.Fatalf("token node_id = %q, want generate_game_master", ev.NodeID)
			}
		case EventError:
			t.Fatalf("unexpected error event: %s", ev.Err)
		}
	}
	if strings.Contains(streamed.String(), "case_action") {
		t.Fatalf("streamed hidden classifier output: %q", streamed.String())
	}
	...
	if len(history) != 2 {
		t.Fatalf("history messages = %d, want user + visible assistant: %+v", len(history), history)
	}
	if history[0].Role != model.RoleUser || history[0].Content() != "我想看看留声机" {
		t.Fatalf("history user = %+v", history[0])
	}
	if history[1].Role != model.RoleAssistant {
		t.Fatalf("history assistant role = %s, want assistant", history[1].Role)
	}
	if got := history[1].Content(); got != "你检查留声机后发现，它被设置成了单曲循环模式。" {
		t.Fatalf("history assistant content = %q", got)
	}
	if got := clawHistoryXMLAttr(history[1], "node", "id"); got != "generate_game_master" {
		t.Fatalf("history assistant node id XML = %q, want generate_game_master", got)
	}
}

func TestRoundTripHistoryParsesSpeakerDirectiveIntoMetadata(t *testing.T) {
	ws, err := workspace.NewLocalWorkspace(t.TempDir())
	if err != nil {
		t.Fatalf("NewLocalWorkspace: %v", err)
	}
	llm := &capturingLLM{replies: []string{`<speaker name="沈知秋" />我那晚确实去过书房。`}}
	app := newTestClaw(t, ws, llm, func(cfg *Config) {
		publishTrue := true
		cfg.History.Enabled = true
		cfg.Agent.Publisher.Nodes = map[string]NodePublishConfig{
		...
		cfg.Agent.Graph = graph.GraphDefinition{
			...
			Edges: []graph.EdgeDefinition{{From: "generate_game_master", To: graph.END}},
		}
	})
	defer app.Close()

	resp, err := app.RoundTrip(Request{Text: "问沈知秋"})
	if err != nil {
		t.Fatalf("RoundTrip: %v", err)
	}
	for {
	...
	if got := history[1].Content(); got != "我那晚确实去过书房。" {
		t.Fatalf("history assistant content = %q", got)
	}
	if got := clawHistoryXMLAttr(history[1], "speaker", "name"); got != "沈知秋" {
		t.Fatalf("history assistant speaker XML = %q, want 沈知秋", got)
	}
	if got := clawHistoryXMLAttr(history[1], "node", "id"); got != "generate_game_master" {
		t.Fatalf("history assistant node XML = %q, want generate_game_master", got)
	}

	board, err := app.boardSeeder().SeedBoard(context.Background(), agent.RunInfo{}, &agent.Request{
		ContextID: app.contextID(),
		Message:   model.NewTextMessage(model.RoleUser, "继续"),
	})
	if err != nil {
...
func TestRoundTripBlankInputUsesProviderSafeUserMessageWithoutChangingClawState(t *testing.T) {
	ws, err := workspace.NewLocalWorkspace(t.TempDir())
	if err != nil {
		t.Fatalf("NewLocalWorkspace: %v", err)
	}
	llm := &capturingLLM{replies: []string{"opening"}}
	app := newTestClaw(t, ws, llm, func(cfg *Config) {
		cfg.History.Enabled = true
		cfg.Agent.Graph = graph.GraphDefinition{
			...
			Nodes: []graph.NodeDefinition{
				{
					...
					Config: map[string]any{
						"source": `
var messages = board.channel(board.MAIN_CHANNEL);
var last = messages[messages.length - 1] || {};
board.setVar("observed_user_text", last.content || "");
`,
					},
				},
				{
			...
			Edges: []graph.EdgeDefinition{
				{From: "capture_user", To: "answer"},
				{From: "answer", To: graph.END},
			},
		}
	})
	defer app.Close()

	resp, err := app.RoundTrip(Request{Text: ""})
	if err != nil {
		t.Fatalf("RoundTrip: %v", err)
	}
	for {
	...
	if last.Role != model.RoleUser || last.Content() != providerSafeEmptyUserText {
		t.Fatalf("last message = (%s, %q), want provider-safe empty user message", last.Role, last.Content())
	}
	history, err := app.history.load(context.Background(), app.contextID())
	if err != nil {
		t.Fatalf("load history: %v", err)
	}
	if len(history) < 1 || history[0].Role != model.RoleUser || history[0].Content() != "" {
		t.Fatalf("history user message = %+v, want empty user input", history)
	}
	st, err := app.loadContextState(context.Background(), app.contextID())
	if err != nil {
		t.Fatalf("loadContextState: %v", err)
	}
	if got := st.Vars["observed_user_text"]; got != "" {
		t.Fatalf("observed_user_text = %q, want empty graph input", got)
	}
}

func TestRoundTripEmitsExtractingStatusBeforeMemorySave(t *testing.T) {
	ws, err := workspace.NewLocalWorkspace(t.TempDir())
	if err != nil {
		t.Fatalf("NewLocalWorkspace: %v", err)
	}
	...
	var sawExtracting, sawResult bool
	for {
		ev, err := resp.Next()
		if errors.Is(err, io.EOF) {
			break
		...
		switch ev.Type {
		case EventStatus:
			if ev.Content == "extracting" {
				if sawResult {
					t.Fatal("extracting status arrived after result")
				}
				sawExtracting = true
			}
		case EventResult:
	...
	if !sawExtracting {
		t.Fatal("missing extracting status event")
	}
	if !sawResult {
		t.Fatal("missing result event")
	}
...
func TestResponseInterruptControlsPartialMemory(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(history) != 2 {
				t.Fatalf("history messages = %d, want user + partial assistant: %+v", len(history), history)
			}
			if got := history[1].Content(); got != "hello " {
				t.Fatalf("history assistant partial = %q, want %q", got, readBeforeInterrupt)
			}
		})
	}
}
