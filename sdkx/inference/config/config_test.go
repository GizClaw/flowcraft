...
func TestDecodeJSONAcceptsVersionedSecretReferences(t *testing.T) {
...
func TestDecodeJSONRejectsInlineSecretsAndUnknownEnvelopeFields(t *testing.T) {
	tests := []struct {
		name string
		data string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := DecodeJSON(strings.NewReader(tt.data)); err == nil {
				t.Fatal("DecodeJSON accepted an unknown secret field")
			}
		})
	}
}
