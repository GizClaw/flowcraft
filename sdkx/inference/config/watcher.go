...
//
// Reloader owns no goroutines: call Run from the caller's lifecycle, or drive
// ReloadOnce directly from an external scheduler.
...
// Runtime returns the current runtime. The returned pointer stays valid after
// later reloads; runtimes are never mutated in place.
...
func (r *Reloader) ReloadOnce(ctx context.Context) error {
	...
	r.mu.RLock()
	unchanged := r.runtime != nil && r.current.Revision == snapshot.Revision
	r.mu.RUnlock()
	if unchanged {
		return nil
	}
	runtime, err := r.builder.NewRuntime(ctx, snapshot.Document, r.options...)
	if err != nil {
		return err
	}
	r.mu.Lock()
	...
	if r.runtime != nil && r.current.Revision == snapshot.Revision {
		return nil
	}
	r.runtime = runtime
