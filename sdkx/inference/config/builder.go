...
// provider/profile Spec, model rules, compiler construction, and validation of
...
func (p ResolvedProfile) Clone() ResolvedProfile {
	...
	for name, secret := range p.Secrets {
		secrets[name] = Secret{value: secret.Bytes()}
	}
	p.Secrets = secrets
	return p
}

// Builder is immutable after construction and safe for concurrent use when
// its factories and resolvers are safe for concurrent use. Catalogs are
// instance-owned; package-global provider registration is intentionally absent.
type Builder struct {
...
func (b *Builder) Build(
	...
	// Runtime construction is local and performs no provider I/O. Running it
	// here catches malformed factory output before definitions escape Builder.
