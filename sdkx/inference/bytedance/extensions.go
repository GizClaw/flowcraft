...
// ---------------------------------------------------------------------------
// Streaming recognition (Doubao ASR V2 SAUC).
// ---------------------------------------------------------------------------

// TranscriptionOptions carries Doubao ASR V2 session settings beyond the
// canonical transcription session config. ITN and punctuation default to on
// in this provider; set them explicitly to disable.
type TranscriptionOptions struct {
	// Provider targets a deployment provider ID other than "bytedance".
	// Attempts for any other provider leave the extension inert rather than
	// rejecting it, so mixed-provider routes keep working.
	Provider string `json:"-"`
	// Diarization enables speaker separation.
	Diarization *bool `json:"diarization,omitempty"`
	// SpeakerNum hints the expected speaker count; implies diarization.
	SpeakerNum *int `json:"speaker_num,omitempty"`
	// Hotwords boosts recognition of domain terms.
	Hotwords []string `json:"hotwords,omitempty"`
	// ResultType selects partial ("single") or full ("full") utterance
	// results.
	ResultType string `json:"result_type,omitempty"`
	// ITN toggles inverse text normalization (digits, punctuation forms).
	ITN *bool `json:"itn,omitempty"`
	// Punctuation toggles automatic punctuation.
	Punctuation *bool `json:"punctuation,omitempty"`
}

func (o TranscriptionOptions) ProviderID() string { return extensionProvider(o.Provider) }
func (o TranscriptionOptions) ExtensionID() string {
	return extensionTranscription
}

func (o TranscriptionOptions) ActiveFields() []inference.ExtensionField {
	var fields []inference.ExtensionField
	if o.Diarization != nil {
		fields = append(fields, "diarization")
	}
	if o.SpeakerNum != nil {
		fields = append(fields, "speaker_num")
	}
	if len(o.Hotwords) > 0 {
		fields = append(fields, "hotwords")
	}
	if o.ResultType != "" {
		fields = append(fields, "result_type")
	}
	if o.ITN != nil {
		fields = append(fields, "itn")
	}
	if o.Punctuation != nil {
		fields = append(fields, "punctuation")
	}
	return fields
}

func (o TranscriptionOptions) Validate() error {
	if o.SpeakerNum != nil && *o.SpeakerNum <= 0 {
		return fmt.Errorf("speaker_num must be positive, not %d", *o.SpeakerNum)
	}
	for index, word := range o.Hotwords {
		if word == "" {
			return fmt.Errorf("hotword %d is empty", index)
		}
	}
	switch o.ResultType {
	case "", "single", "full":
	default:
		return fmt.Errorf("result_type must be single or full, not %q", o.ResultType)
	}
	return nil
}

func (o TranscriptionOptions) Clone() inference.Extension {
	o.Diarization = clonePointer(o.Diarization)
	o.SpeakerNum = clonePointer(o.SpeakerNum)
	o.Hotwords = append([]string(nil), o.Hotwords...)
	o.ITN = clonePointer(o.ITN)
	o.Punctuation = clonePointer(o.Punctuation)
	return o
}

// ---------------------------------------------------------------------------
// Full-duplex dialogue (Doubao realtime duplex).
// ---------------------------------------------------------------------------

// RealtimeOptions carries Doubao realtime duplex session settings beyond the
// canonical realtime config. Rates are percentage offsets: 0 keeps the voice
// default.
type RealtimeOptions struct {
	...
	// rejecting it, so mixed-provider routes keep working.
	Provider string `json:"-"`
	// OutputSpeed adjusts the spoken response pace.
	OutputSpeed *int `json:"output_speed,omitempty"`
	// OutputLoudness adjusts the spoken response volume.
	OutputLoudness *int `json:"output_loudness,omitempty"`
}

func (o RealtimeOptions) ProviderID() string  { return extensionProvider(o.Provider) }
func (o RealtimeOptions) ExtensionID() string { return extensionRealtime }

func (o RealtimeOptions) ActiveFields() []inference.ExtensionField {
	var fields []inference.ExtensionField
	if o.OutputSpeed != nil {
		fields = append(fields, "output_speed")
	}
	if o.OutputLoudness != nil {
		fields = append(fields, "output_loudness")
	}
	return fields
}

func (o RealtimeOptions) Validate() error { return nil }

func (o RealtimeOptions) Clone() inference.Extension {
	o.OutputSpeed = clonePointer(o.OutputSpeed)
	o.OutputLoudness = clonePointer(o.OutputLoudness)
	return o
}
