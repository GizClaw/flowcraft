...
// provider config. It validates the provider Spec, merges the model catalog,
// resolves every credential profile, and binds each catalog model to the
// openers its kind serves. Unknown models fail closed: only catalog models
// (built-in or declared via Spec.Models) are exposed.
	...
	case kindASR:
		return inference.Openers{
			Transcription: func(
				...
				return openASR(cls, spec, id, model.Profile)
			},
		}
	case kindRealtime:
		return inference.Openers{
			Realtime: func(
				...
				return openRealtime(cls, spec, id, model.Profile)
			},
		}
	}
	return inference.Openers{}
