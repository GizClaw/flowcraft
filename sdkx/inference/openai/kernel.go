...
// KernelTranscription binds the audio transcription driver (unary only)
// for one deployment.
func KernelTranscription(
	client openaigo.Client,
	model string,
) (inference.TranscriptionOperations, error) {
	unary, err := inference.BindTranscription(
		compileTranscription(model),
		transportTranscription(client),
		decodeTranscription,
	)
	if err != nil {
		return inference.TranscriptionOperations{}, err
	}
	return inference.TranscriptionOperations{Unary: unary}, nil
}
