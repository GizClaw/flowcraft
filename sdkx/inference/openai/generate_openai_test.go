...
func TestProfileMaterial(t *testing.T) {
	t.Run("missing api key", func(t *testing.T) {
		_, err := newProfileMaterial(config.ResolvedProfile{ID: "default"})
		if err == nil {
...
func TestFactoryBuild(t *testing.T) {
	...
	var whisper *inference.ModelImplementation
	for index := range provider.Models {
		if provider.Models[index].Descriptor.ID.Name == "whisper-1" {
			whisper = &provider.Models[index]
		}
	}
	if whisper == nil {
		t.Fatal("whisper-1 missing from provider models")
	}
	if whisper.Descriptor.Lifecycle.Status != inference.ModelStatusDeprecated {
		t.Fatalf("whisper lifecycle = %+v", whisper.Descriptor.Lifecycle)
	}
	if whisper.Descriptor.Lifecycle.Replacement == nil ||
