...
func TestConformanceTranscriptionCompiler(t *testing.T) {
	clip, err := media.NewAudioBytes(make([]byte, 640), "audio/wav")
	if err != nil {
		t.Fatal(err)
	}
	transcriptionRequest := func() inference.TranscriptionRequest {
		return inference.TranscriptionRequest{Audio: clip}
	}

	inferencetest.RunCompiler(t, inferencetest.CompilerSuite[inference.TranscriptionRequest, sttWire]{
		...
		Fields: func(request inference.TranscriptionRequest) []inference.FieldID {
			return request.ActiveFields()
		},
		Compile: compileTranscription("gpt-4o-transcribe"),
		AssertWire: func(t *testing.T, wire sttWire) {
			if wire.model != "gpt-4o-transcribe" || len(wire.data) != 640 {
				t.Fatalf("wire = %+v", wire)
			}
			if wire.filename != "audio.wav" {
				t.Fatalf("wire filename = %q", wire.filename)
			}
		},
		Rejections: []inferencetest.CompilerRejection[inference.TranscriptionRequest]{
			{
				Name: "URL audio has no fetcher",
				Request: func() inference.TranscriptionRequest {
					url, err := media.NewAudioURL("https://example.com/a.mp3", "audio/mpeg")
					if err != nil {
						t.Fatal(err)
					}
					return inference.TranscriptionRequest{Audio: url}
				},
				Field: inference.FieldTranscriptionAudio,
				Kind:  inference.UnsupportedFeature,
			},
		},
	})
