// Package sandbox provides opinionated assembly helpers for concrete
// sandbox backends. The policy contracts and decorators live in
// sdk/sandbox; this sdkx package only wires them into recommended
// deployment shapes.
package sandbox

import coresandbox "github.com/GizClaw/flowcraft/sdk/sandbox"
...
type LocalPolicy struct {
	...
	Approval        coresandbox.ApprovalFunc
...
func DefaultLocalPolicy(root string, approval coresandbox.ApprovalFunc, sensitiveCommands ...string) LocalPolicy {
	predicates := []coresandbox.Predicate{
		coresandbox.WorkDirOutsideRoot(root),
		coresandbox.NetNonDefault(),
	}
	if len(sensitiveCommands) > 0 {
		predicates = append(predicates, coresandbox.CommandPatterns(sensitiveCommands...))
	}
	return LocalPolicy{
		Approval:   approval,
...
func ComposeLocal(backend coresandbox.Runner, policy LocalPolicy) coresandbox.Runner {
	runner := backend
	if policy.AllowedCommands != nil {
		runner = coresandbox.AllowCommands(runner, policy.AllowedCommands)
	}
	if len(policy.Predicates) > 0 {
		runner = coresandbox.WithApproval(
			runner,
			policy.Approval,
			policy.Predicates...,
		)
	}
	return coresandbox.WithDefaults(runner, policy.Defaults)
}
