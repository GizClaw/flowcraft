...
// Deps supplies application-owned resources that YAML cannot express.
type Deps struct {
...
type Factory func(ctx context.Context, input FactoryInput) (coresandbox.Runner, error)

// Builder owns an instance-local backend factory registry.
type Builder struct {
	deps      Deps
	factories map[string]Factory
}

// NewBuilder creates a builder with local, seatbelt, and nsjail registered.
...
func (b *Builder) RegisterFactory(backend string, factory Factory) error {
	...
	if backend == "" {
		return errdefs.Validationf("sandbox config: backend name is empty")
	}
	if factory == nil {
		return errdefs.Validationf(
			"sandbox config: factory for backend %q is nil", backend)
	}
	if _, exists := b.factories[backend]; exists {
		return errdefs.Validationf(
			"sandbox config: backend %q is already registered", backend)
	}
	b.factories[backend] = factory
	return nil
}
...
func (b *Builder) Build(ctx context.Context, doc Document) (_ *Registry, err error) {
	...
	for _, name := range sortedKeys(doc.Sandboxes) {
		...
		factory, ok := b.factories[entry.Backend]
		if !ok {
			return nil, errdefs.Validationf(
		...
		backend, err := factory(ctx, FactoryInput{
			Root:     root,
			Settings: entry.Settings.Node(),
		})
		if err != nil {
		...
		runners[name] = sandboxx.ComposeLocal(backend, policy)
	}
