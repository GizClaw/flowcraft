...
func TestBuildRejectsWorkspaceBackendAndSettingsFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"local unknown setting", "{backend: local, workspace: project, settings: {bogus: true}}", "field bogus"},
		{"local negative output", "{backend: local, workspace: project, settings: {default_max_output_bytes: -1}}", "non-negative"},
		{"seatbelt unknown setting", "{backend: seatbelt, workspace: project, settings: {bogus: true}}", "field bogus"},
		{"nsjail unknown setting", "{backend: nsjail, workspace: project, settings: {bogus: true}}", "field bogus"},
		{"nsjail relative path escape", "{backend: nsjail, workspace: project, settings: {writable_paths: [../escape]}}", "escapes workspace root"},
	}
...
func TestBuildRejectsRelativeWritablePathSymlinkEscape(t *testing.T) {
	root := t.TempDir()
	outside := t.TempDir()
...
func TestNSJailUnsupportedPlatformClassification(t *testing.T) {
	if runtime.GOOS == "linux" {
		t.Skip("nsjail support depends on the host binary on Linux")
	}
	root := t.TempDir()
	doc := mustParse(t, `
version: v1
sandboxes:
  x: {backend: nsjail, workspace: project}
`)
	_, err := sandboxconfig.NewBuilder(sandboxconfig.Deps{
		Workspaces: buildWorkspaces(t, root, false),
	}).Build(context.Background(), doc)
...
func TestCustomFactoryReceivesRootAndOpaqueSettings(t *testing.T) {
	...
	err := builder.RegisterFactory("custom", func(_ context.Context, input sandboxconfig.FactoryInput) (sandbox.Runner, error) {
		...
		value, err := sandboxconfig.DecodeSettings[settings](input.Settings)
		if err != nil {
			return nil, err
		}
