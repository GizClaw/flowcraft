...
func TestRegisterRejectsUnknownSettingsAndEscape(t *testing.T) {
	...
	if err := nsjail.Register(builder); err != nil {
		t.Fatalf("Register: %v", err)
	}
	for name, entry := range map[string]string{
		"unknown setting": "{backend: nsjail, workspace: project, settings: {bogus: true}}",
		"path escape":     "{backend: nsjail, workspace: project, settings: {writable_paths: [../escape]}}",
	} {
