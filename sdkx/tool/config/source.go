...
// registry the Builder owns is the catalog, and a source's job is to
...
//
// No kind is registered by default. A source pulls in an external
// dependency (the MCP SDK, an RPC client), so the host opts in
...
func (b *Builder) buildSources(ctx context.Context, entries []SourceEntry) ([]Source, error) {
	...
	for i, entry := range entries {
		...
		if err := source.Attach(ctx, b.registry); err != nil {
			_ = source.Close()
			closeAttached()
			return nil, fmt.Errorf(
