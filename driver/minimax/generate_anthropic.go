...
func wireToParams(wire generateWire) anthropicgo.MessageNewParams {
	...
	case wire.effort != "":
		// Unreachable on this dialect — the compiler turns every effort
		// request into binary thinking — kept so the wire model stays
		// faithful to the Messages protocol.
		params.OutputConfig.Effort = anthropicgo.OutputConfigEffort(wire.effort)
	case wire.thinking != nil && *wire.thinking:
		// MiniMax serves the binary-thinking dialect: the reasoning intent
		// compiles to thinking: {type: "adaptive"}.
		params.Thinking = anthropicgo.ThinkingConfigParamUnion{
...
func messageToRaw(message *anthropicgo.Message) (generateRaw, error) {
	if message == nil {
		return generateRaw{}, fmt.Errorf("minimax: empty message object")
	}
