...
> `sdkx` follows as a coordinated release pinned to `sdk v0.5.0` and is
> documented in the [sdkx section](#sdkx-v050-provider--adapter-rebuild)
> below; `memory` awaits its own pin bump.
...
| `sdk/script`                                                                                        | `sdk/agent/bindings` (host bindings); runtimes move to `sdkx/agent/jsrt` and `sdkx/agent/luart` (relocated to `sdkx/agent/script/...` in v0.5.4, old paths kept as deprecated aliases)                                        |
...
The same window rebuilds the provider/adaptor layer. `sdkx` follows `sdk`
with a release pinned to `sdk v0.5.0`; the full removed / new surface and
the sdkx-specific upgrade path are documented in the
[sdkx v0.5.0 section](#sdkx-v050-provider--adapter-rebuild) below.
...
## sdkx v0.5.0: provider / adapter rebuild
...
cutover documented above. Four themes drive it:
...
### sdkx module surface changes (vs `sdkx/v0.4.x`)
...
- This guide documents the SDK-side cutover (`sdk/engine`, `sdk/llm`,
  `sdk/embedding`, `sdk/model`, `sdk/script` → `sdk/agent` + `sdk/inference` +
  `sdk/message` + `sdk/config`).
...
- **`sdkx/v0.5.0`** (coordinated): provider/adaptor rebuild pinned to
  `sdk v0.5.0`; see the [sdkx section](#sdkx-v050-provider--adapter-rebuild).
- **`memory`** (pending): awaits its own coordinated pin bump to
  `sdk v0.5.0`; see the memory module's release notes.
