...
// the stdlib `sync` toolkit for memory-internal use.
//
// This package is INTERNAL by design — it deliberately sits under
// memory/internal/ so external modules cannot take a dependency on
...
// motivation is per-conversation Append (memory/history) and per-
// namespace history-store fallback (memory/recall), where two
