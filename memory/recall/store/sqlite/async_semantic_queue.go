...
func (q *asyncSemanticQueue) Enqueue(ctx context.Context, job port.AsyncSemanticJob) (port.AsyncSemanticReceipt, error) {
	...
	err = tx.QueryRowContext(ctx, `SELECT enqueued_at_ns FROM recall_async_semantic_jobs WHERE request_id = ?`, job.RequestID).Scan(&enqueuedAt)
	if err == nil {
	...
	if _, err := tx.ExecContext(ctx,
		fmt.Sprintf(`INSERT INTO recall_async_semantic_jobs(request_id, runtime_id, user_id, status, enqueued_at_ns, lease_until_ns, lease_token, attempt, failure_class, failure_err, result_json, payload_json) VALUES(%s)`, phs(1, 12)),
...
func (q *asyncSemanticQueue) Requeue(ctx context.Context, job port.AsyncSemanticJob) (port.AsyncSemanticReceipt, bool, error) {
	...
	if err == sql.ErrNoRows {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE recall_async_semantic_jobs
			SET runtime_id = ?, user_id = ?, status = ?, lease_until_ns = NULL, lease_token = '', attempt = 0, failure_class = '', failure_err = '', result_json = '', payload_json = ?
			WHERE request_id = ?`,
...
func (q *asyncSemanticQueue) Cancel(ctx context.Context, requestID string) error {
	...
	err = tx.QueryRowContext(ctx, `SELECT runtime_id, user_id, status FROM recall_async_semantic_jobs WHERE request_id = ?`, requestID).Scan(&runtimeID, &userID, &status)
...
func (q *asyncSemanticQueue) CancelScope(ctx context.Context, scope domain.Scope) (int, error) {
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT request_id FROM recall_async_semantic_jobs WHERE runtime_id = ? AND user_id = ? AND status <> ?`, runtimeID, userID, sqlstmt.StatusComplete)
	if err != nil {
		return 0, err
	}
...
func (q *asyncSemanticQueue) PurgeScope(ctx context.Context, scope domain.Scope) (int, error) {
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT request_id FROM recall_async_semantic_jobs WHERE runtime_id = ? AND user_id = ?`, runtimeID, userID)
	if err != nil {
		return 0, err
	}
	ids, err := scanStrings(rows)
...
func (q *asyncSemanticQueue) CancelMatchingEpisodes(ctx context.Context, scope domain.Scope, deletedEpisodeFactIDs []string) (int, error) {
	...
	rows, err := tx.QueryContext(ctx,
		fmt.Sprintf(`SELECT DISTINCT j.request_id FROM recall_async_semantic_jobs j
			...
			WHERE j.runtime_id = ? AND j.user_id = ? AND j.status <> ? AND e.episode_fact_id IN (%s)`, phs(1, len(targets))),
		args...)
	if err != nil {
		return 0, err
	}
...
func (q *asyncSemanticQueue) Claim(ctx context.Context, opts port.AsyncSemanticClaimOptions) ([]port.AsyncSemanticJob, error) {
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `UPDATE recall_async_semantic_jobs SET status = ?, lease_token = '' WHERE status = ? AND lease_until_ns IS NOT NULL AND lease_until_ns <= ?`, sqlstmt.StatusPending, sqlstmt.StatusLeased, now.UnixNano()); err != nil {
		return nil, err
	}
	where, args := q.claimWhere(opts, now)
	rows, err := tx.QueryContext(ctx,
		fmt.Sprintf(`SELECT request_id, payload_json FROM recall_async_semantic_jobs WHERE %s ORDER BY enqueued_at_ns ASC, runtime_id ASC, user_id ASC, request_id ASC LIMIT %d`, where, opts.Max),
		args...)
	if err != nil {
		return nil, err
	}
	...
	for _, r := range selected {
		...
		if _, err := tx.ExecContext(ctx, `UPDATE recall_async_semantic_jobs SET status = ?, lease_until_ns = ?, lease_token = ?, attempt = ?, payload_json = ? WHERE request_id = ? AND status = ?`,
...
func (q *asyncSemanticQueue) Stats(ctx context.Context, filter port.AsyncSemanticStatsFilter) (port.AsyncSemanticStats, error) {
	...
	rows, err := q.b.db.QueryContext(ctx, `SELECT status, lease_until_ns, failure_class FROM recall_async_semantic_jobs WHERE runtime_id = ? AND user_id = ?`, runtimeID, userID)
	if err != nil {
	...
	out.CancelledTotal, err = counter(ctx, q.b.db, sqlstmt.CounterAsyncSemantic, filter.Scope)
	return out, err
}

func (q *asyncSemanticQueue) replaceEpisodeRowsTx(ctx context.Context, tx *sql.Tx, job port.AsyncSemanticJob) error {
	if _, err := tx.ExecContext(ctx, `DELETE FROM recall_async_semantic_job_episodes WHERE request_id = ?`, job.RequestID); err != nil {
		return err
	}
	runtimeID, userID := sqlstmt.ScopeParts(job.Scope)
	for _, id := range sqlstmt.UniqueNonEmpty(job.EpisodeFactIDs) {
		if _, err := tx.ExecContext(ctx, `INSERT INTO recall_async_semantic_job_episodes(request_id, runtime_id, user_id, episode_fact_id) VALUES(?,?,?,?)`, job.RequestID, runtimeID, userID, id); err != nil {
			return err
		}
	}
	return nil
...
func (q *asyncSemanticQueue) pendingDepthTx(ctx context.Context, tx *sql.Tx, scope domain.Scope) (int, error) {
	runtimeID, userID := sqlstmt.ScopeParts(scope)
	var n int
	err := tx.QueryRowContext(ctx, `SELECT COUNT(*) FROM recall_async_semantic_jobs WHERE runtime_id = ? AND user_id = ? AND status = ?`, runtimeID, userID, sqlstmt.StatusPending).Scan(&n)
	return n, err
}

func (q *asyncSemanticQueue) deleteRequestTx(ctx context.Context, tx *sql.Tx, requestID string) error {
	if _, err := tx.ExecContext(ctx, `DELETE FROM recall_async_semantic_job_episodes WHERE request_id = ?`, requestID); err != nil {
		return err
	}
	_, err := tx.ExecContext(ctx, `DELETE FROM recall_async_semantic_jobs WHERE request_id = ?`, requestID)
	return err
}

func (q *asyncSemanticQueue) deleteRequestsTx(ctx context.Context, tx *sql.Tx, requestIDs []string) error {
...
func (q *asyncSemanticQueue) mutateLeased(ctx context.Context, requestID, leaseToken string, mutate func(*port.AsyncSemanticJob) (string, port.AsyncSemanticFailure, string, error)) error {
	...
	err = tx.QueryRowContext(ctx, `SELECT status, lease_token, payload_json FROM recall_async_semantic_jobs WHERE request_id = ?`, requestID).Scan(&status, &currentToken, &raw)
	if err == sql.ErrNoRows || status == sqlstmt.StatusComplete {
		return tx.Commit()
	}
	if err != nil {
	...
	_, err = tx.ExecContext(ctx,
		`UPDATE recall_async_semantic_jobs SET status = ?, lease_until_ns = ?, lease_token = '', failure_class = ?, failure_err = ?, result_json = ?, payload_json = ? WHERE request_id = ?`,
