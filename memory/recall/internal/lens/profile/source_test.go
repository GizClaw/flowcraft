...
func TestSource_AgentScopedQueryStillHonorsBudget(t *testing.T) {
	...
	if len(res.Candidates) != 1 {
		t.Fatalf("agent-scoped profile source must honor budget, got %+v", res.Candidates)
	}
	if !res.Truncated {
		t.Fatal("expected truncated profile result for over-budget agent-scoped query")
	}
}
