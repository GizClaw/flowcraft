...
| **FlowCraft (this repo, 25921980006)**         |          **83.66** | LoCoMo10 n=1542, **same judge protocol** but extractor=DeepSeek-V4-Flash; per-cat single-hop=80.14 multi-hop=67.71 open-domain=87.16 temporal=82.24 — see [§ Extractor-swap ablation (May 2026)](#extractor-swap-ablation-may-2026) |
...
#### Robustness against open framework bugs

Run `25921980006` is anchored to commit `db16b7f2` on
`feat/entity-recall-lane`. There are 18 open bug tickets against
`sdk/recall` and `sdk/retrieval` at the time of this run. Each was
triaged against the eval's code path before the number was
published:

|                              Issue | Class                                                                       |                                               Triggered by 25921980006?                                                |
| ---------------------------------: | --------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------: |
|                               #148 | `runEntityLinkRecall` bypasses request Filter (entity-link lane leak)       | ❌ A+B is **off** in this run; the lane is not in the pipeline. (#148 **explains** the A+B regression at 25914193558.) |
|                               #163 | `Memory.Add` does not write content_hash → bypasses dedup                   |                                             ❌ eval uses `Save`, not `Add`                                             |
|                               #156 | `LayerRetriever` silently downgrades ModeVector → BM25                      |                                   ❌ eval uses `recall.Memory`, not `LayerRetriever`                                   |
|                               #158 | `dedupHashes` ignores tombstone (resolver-erased facts come back as no-ops) |                                 ❌ `--update-resolver` is empty (disabled) in this run                                 |
|                               #149 | `SaveAsync` ignores `WithHistoryStore`                                      |                                            ❌ eval uses synchronous `Save`                                             |
|                               #150 | `Scope.Partitions` documented but never consulted by Recall                 |                                           ❌ eval does not set `Partitions`                                            |
|                               #151 | TTL sweeper does not call `EntityStore.Forget`                              |                                               ❌ TTL sweeper not enabled                                               |
|                               #164 | `Memory.Rollback` does not sync `EntityStore`                               |                                            ❌ `Rollback` not called by eval                                            |
|                               #165 | `jobs.handleJob` skips `validateScope`                                      |                                                ❌ no durable job queue                                                 |
|                               #161 | `HybridShortCircuit` on hybrid-capable backend                              |                             ❌ no in-tree backend implements `Hybridable` (per issue body)                             |
|                               #153 | `FileStore.SaveMessages` silently drops writes                              |                                          ❌ eval uses in-memory history store                                          |
|                               #154 | `appendHistory` self-race on InMemoryStore                                  |                     🟡 possible at `ingest_concurrency=16` but bounded to ≤0.x pp turn-loss noise                      |
|                               #162 | history compactor races outside per-conversation worker                     |                                              🟡 same noise class as #154                                               |
| #145, #152, #155, #157, #159, #160 | knowledge / journal / godoc / panic-path bugs                               |                                                ❌ not on eval code path                                                |

The two 🟡 rows together cap residual contamination at well under
1 pp — they affect history-append ordering, not retrieval or
extraction. **None of the bugs that would shift a 7 pp gap are on
this run's code path.** The reason: the eval intentionally
exercises only the well-trodden subset (synchronous `Save`,
single-namespace `recall.Memory`, no `update-resolver`, no entity
lane, no TTL sweep, no `Rollback`, no `Partitions`, no durable
queue, no hybrid backend). The improvement is from the extractor
LLM, not from a path that touches an open ticket.

A variance-check rerun of run `25921980006` configuration is
queued to confirm the number reproduces within ±0.5 pp before
...
| **FlowCraft (best operational)**           |                          **83.66** J% (extractor=DeepSeek-V4-Flash; answer=judge=gpt-4o-mini) | Same as above except `--extractor-llm=azure_ds_flash`. Answer + judge LLMs remain gpt-4o-mini — apples-to-apples on the published judge protocol.                                                                                                                                                    | run [25921980006](https://github.com/GizClaw/flowcraft/actions/runs/25921980006) (May 15, 2026); n=1542. Per-category: single-hop 80.14, temporal 82.24, multi-hop 67.71, open-domain 87.16, adversarial 100.00. **Single-run; variance-check rerun pending before external citation.** See [§ Extractor-swap ablation (May 2026)](#extractor-swap-ablation-may-2026) for the four-cell ablation and the open-bug triage that confirms no eval-path contamination. |
