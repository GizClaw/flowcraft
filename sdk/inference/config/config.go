// Package config loads versioned, secret-reference-only inference
// configuration and materializes immutable inference provider
// definitions.
//
// Provider-specific packages own the interpretation of
// ProviderConfig.Spec and ProfileConfig.Spec. This package deliberately
// does not define model capability flags or provider wire rules.
package config
...
type Document struct {
	...
	// validates every target against the assembled runtime at build time,
	// so a snapshot referencing unknown or retired models fails fast.
...
// ProfileConfig declares one credential profile. Secrets contains
// references, never resolved values. Spec contains provider-owned,
// non-secret settings such as endpoints, organizations, or projects.
...
// SecretRef identifies a value in an explicitly supplied
// SecretResolver.
...
// Parse strictly decodes and validates one inference configuration
// document. YAML and JSON are both accepted; unknown fields, trailing
// documents, inline credentials, and invalid values are all errors.
func Parse(data []byte) (Document, error) {
	document, err := utils.Decode[Document](data)
	if err != nil {
...
// Provider configuration types should deliberately omit credentials:
// resolved secrets arrive separately through ResolvedProfile.Secrets.
