...
//
// Chain order matters: mws[0] is outermost (sees the call first and
// the result last), matching engine.ComposeHost. A bare Executor has
// no panic recovery: production chains should start with a recover
// middleware so a panicking tool cannot take down an ExecuteAll
// fan-out goroutine.
...
// are skipped. catalog must be non-nil; a nil catalog is a
