...
//
// It is a derived exploration view over canonical facts: typed relation
// edges and bounded co-occurrence edges. It is not a truth layer.
package graph
...
type shard struct {
	...
	reverse map[string][]directedEdge // factID -> edges for Forget
}
		...
		for _, node := range frontier {
			...
			for to, refs := range neighbors {
				...
				for _, ref := range refs {
					...
					seenFact[ref.factID] = struct{}{}
					collected = append(collected, scored{hop: hop, factID: ref.factID})
				}
				if !reached {
					continue
				}
...
// CapGraphHops clamps caller-supplied expansion depth to the default
// bounded expansion (docs §8.4 Phase 7).
