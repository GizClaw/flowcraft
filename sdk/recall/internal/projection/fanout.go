...
const (
	OpProject = port.OpProject
	OpForget  = port.OpForget
	OpRebuild = port.OpRebuild
)
...
func New(projections []port.Projection, hook port.TelemetryHook) *Fanout {
	if hook == nil {
...
func (f *Fanout) ProjectRequired(ctx context.Context, facts []domain.TemporalFact) error {
	...
	return f.runRequired(ctx, OpProject, len(facts), func(p port.Projection) error {
...
func (f *Fanout) ProjectOptional(ctx context.Context, facts []domain.TemporalFact) {
	...
	f.runOptional(ctx, OpProject, len(facts), func(p port.Projection) error {
		return p.Project(ctx, facts)
	})
}
...
func (f *Fanout) ForgetRequired(ctx context.Context, scope domain.Scope, factIDs []string) error {
	...
	return f.runRequired(ctx, OpForget, len(factIDs), func(p port.Projection) error {
...
func (f *Fanout) ForgetOptional(ctx context.Context, scope domain.Scope, factIDs []string) {
	...
	f.runOptional(ctx, OpForget, len(factIDs), func(p port.Projection) error {
		return p.Forget(ctx, scope, factIDs)
	})
}
...
func (f *Fanout) RebuildRequired(ctx context.Context, scope domain.Scope, facts []domain.TemporalFact) error {
	...
	return f.runRequired(ctx, OpRebuild, len(facts), func(p port.Projection) error {
...
func (f *Fanout) RebuildOptional(ctx context.Context, scope domain.Scope, facts []domain.TemporalFact) {
	...
	f.runOptional(ctx, OpRebuild, len(facts), func(p port.Projection) error {
		return p.Rebuild(ctx, scope, facts)
	})
}
...
func (f *Fanout) runRequired(ctx context.Context, op port.Op, n int, call func(port.Projection) error) error {
	for _, p := range f.required {
		if err := ctx.Err(); err != nil {
			return err
		}
		err := call(p)
		f.telemetry.OnProjection(port.ProjectionEvent{
			Projection:  p.Name(),
			Op:          op,
...
func (f *Fanout) runOptional(ctx context.Context, op port.Op, n int, call func(port.Projection) error) {
