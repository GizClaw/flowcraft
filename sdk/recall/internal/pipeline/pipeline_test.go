...
// bestEffortStage returns a BestEffort-wrapped error from Run so the
// framework records Status=Degraded but keeps iterating.
type bestEffortStage struct {
	name string
	err  error
	// wrap, when non-nil, re-wraps the BestEffort value through it
	// (e.g. fmt.Errorf("ctx: %w", ...)) so we can prove errors.As
	// still detects BestEffortFailure across user wrapping.
	wrap func(error) error
}

func (s bestEffortStage) Name() string { return s.name }
func (s bestEffortStage) Run(_ context.Context, st *testState) (diagnostic.StageDetail, error) {
	st.runOrder = append(st.runOrder, s.name)
	err := pipeline.BestEffort(s.err)
	if s.wrap != nil && err != nil {
		err = s.wrap(err)
	}
	return nil, err
}

// bestEffortCompensatingStage exposes a Compensator the test asserts
// is NOT invoked when Run returns a BestEffortFailure.
type bestEffortCompensatingStage struct {
	name string
	err  error
}

func (s bestEffortCompensatingStage) Name() string { return s.name }
func (s bestEffortCompensatingStage) Run(_ context.Context, st *testState) (diagnostic.StageDetail, error) {
	st.runOrder = append(st.runOrder, s.name)
	return nil, pipeline.BestEffort(s.err)
}
func (s bestEffortCompensatingStage) Compensate(_ context.Context, st *testState) error {
	st.compensated = append(st.compensated, s.name)
	return nil
}
...
func TestPipeline_BestEffortFailure_StatusDegraded(t *testing.T) {
	...
	p := newPipeline([]pipeline.Stage[*testState]{
		bestEffortStage{name: "s1", err: boom},
		fakeStage{name: "s2"},
	}, hook)

	if err := p.Run(context.Background(), st); err != nil {
		t.Fatalf("Run must return nil for best-effort failure, got %v", err)
	}
	if got := st.runOrder; len(got) != 2 || got[0] != "s1" || got[1] != "s2" {
		t.Fatalf("runOrder = %v, want [s1 s2] (pipeline must continue past degraded)", got)
	}
	if len(st.stages) != 2 {
		t.Fatalf("trace.Stages len = %d, want 2", len(st.stages))
	}
	if st.stages[0].Status != diagnostic.StatusDegraded {
		t.Errorf("s1 Status = %q, want degraded", st.stages[0].Status)
	}
	if st.stages[0].Err != "boom" {
		t.Errorf("s1 Err = %q, want %q", st.stages[0].Err, "boom")
	}
	if st.stages[1].Status != diagnostic.StatusOK {
		t.Errorf("s2 Status = %q, want ok", st.stages[1].Status)
	}
	if len(hook.events) != 2 {
		t.Fatalf("hook events = %d, want 2", len(hook.events))
	}
	if hook.events[0].Status != diagnostic.StatusDegraded {
		t.Errorf("hook[0] Status = %q, want degraded", hook.events[0].Status)
	}
}

// TestPipeline_BestEffortFailure_DoesNotTriggerCompensator pins the
// "best-effort failure is a non-failure outcome" semantic: even when
// the degraded stage has a Compensator, the framework MUST NOT call
// it, mirroring the ShortCircuit contract.
func TestPipeline_BestEffortFailure_DoesNotTriggerCompensator(t *testing.T) {
	hook := newCaptureHook()
	st := &testState{}
	p := newPipeline([]pipeline.Stage[*testState]{
		compensatingStage{name: "s1"},
		bestEffortCompensatingStage{name: "s2", err: errors.New("optional projection down")},
		compensatingStage{name: "s3"},
	}, hook)
	...
	if got := st.runOrder; len(got) != 3 {
		t.Fatalf("runOrder = %v, want all three stages", got)
	}
	if len(st.compensated) != 0 {
		t.Errorf("compensators must NOT run on BestEffortFailure, got %v", st.compensated)
	}
	if st.stages[1].Status != diagnostic.StatusDegraded {
		t.Errorf("s2 Status = %q, want degraded", st.stages[1].Status)
	}
}

// TestPipeline_BestEffortFailure_PropagatesThroughWrap proves the
// framework uses errors.As (not a type assertion) so callers can
// wrap the BestEffortFailure with fmt.Errorf("...: %w", ...) and
// the Degraded classification still fires.
func TestPipeline_BestEffortFailure_PropagatesThroughWrap(t *testing.T) {
	hook := newCaptureHook()
	st := &testState{}
	inner := errors.New("embedder offline")
	p := newPipeline([]pipeline.Stage[*testState]{
		bestEffortStage{
			name: "s1",
			err:  inner,
			wrap: func(e error) error { return fmt.Errorf("ctx: %w", e) },
		},
		fakeStage{name: "s2"},
	}, hook)

	if err := p.Run(context.Background(), st); err != nil {
		t.Fatalf("Run = %v, want nil even with wrapped BestEffortFailure", err)
	}
	if st.stages[0].Status != diagnostic.StatusDegraded {
		t.Fatalf("s1 Status = %q, want degraded (errors.As must traverse wrap)", st.stages[0].Status)
	}
	// The diag carries the inner err string (BestEffortFailure.Err.Error()),
	// not the outer wrap, so dashboards see the root cause directly.
	if st.stages[0].Err != "embedder offline" {
		t.Errorf("s1 Err = %q, want %q", st.stages[0].Err, "embedder offline")
	}
	if st.stages[1].Stage != "s2" || st.stages[1].Status != diagnostic.StatusOK {
		t.Errorf("s2 not reached after wrapped degraded failure: %+v", st.stages[1])
	}
}

// TestPipeline_BestEffort_NilReturnsNil confirms the BestEffort
// helper is safe to call unconditionally — a nil input must produce
// a nil error so stages can write `return d, pipeline.BestEffort(err)`
// without branching on err.
func TestPipeline_BestEffort_NilReturnsNil(t *testing.T) {
	if got := pipeline.BestEffort(nil); got != nil {
		t.Errorf("BestEffort(nil) = %v, want nil", got)
	}
}
