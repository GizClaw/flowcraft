...
func TestDefaultStructurizer_KindFallbackIsNote(t *testing.T) {
	...
	for _, content := range cases {
		t.Run(content, func(t *testing.T) {
			f := model.TemporalFact{Content: content}
			out := DefaultStructurizer{}.Structurize(f, Input{})
			if out.Kind != model.KindNote {
...
func TestDefaultStructurizer_HonoursCustomEntityExtractor(t *testing.T) {
	...
	f := model.TemporalFact{Content: "Alice met Bob at Paris."}
	out := s.Structurize(f, Input{})
...
func (s stubEntityExtractor) ExtractEntities(string, []EntitySnapshot) []string {
...
func TestDiffStructurizerCoverage_OnlyCountsEmptyToNonEmpty(t *testing.T) {
	cases := []struct {
		name   string
		before model.TemporalFact
		after  model.TemporalFact
	...
	}{
		{
			...
			before: model.TemporalFact{},
			after: model.TemporalFact{
				Kind: model.KindState, Subject: "alice", Entities: []string{"alice"},
				Metadata: map[string]any{MetaValidFromHint: "2024-05-07"},
			},
			want: StructurizerCoverage{TotalFactsSeen: 1, KindFilled: 1, EntitiesFilled: 1, SubjectFilled: 1, ValidFromHintFilled: 1},
		},
		{
			name: "caller_supplied_kind_not_counted",
			before: model.TemporalFact{
				Kind: model.KindRelation, Subject: "bob", Entities: []string{"bob"},
			...
			want: StructurizerCoverage{TotalFactsSeen: 1},
		},
		{
			name:   "partial_fill_subject_only",
			before: model.TemporalFact{Kind: model.KindNote, Entities: []string{"x"}},
			after:  model.TemporalFact{Kind: model.KindNote, Subject: "alice", Entities: []string{"x"}},
			want:   StructurizerCoverage{TotalFactsSeen: 1, SubjectFilled: 1},
		},
	}
...
func TestDefaultStructurizer_TrustsLLMSuppliedKind(t *testing.T) {
	...
	out := DefaultStructurizer{}.Structurize(f, Input{})
	if out.Kind != model.KindState {
		t.Errorf("LLM-supplied Kind overwritten by keyword fallback: got %q want %q", out.Kind, model.KindState)
	}
}
...
func TestDefaultStructurizer_KeepsCallerSuppliedFields(t *testing.T) {
	...
	out := DefaultStructurizer{}.Structurize(pre, Input{})
	if out.Kind != model.KindRelation || out.Subject != "Bob" || out.Predicate != "spouse" || out.Object != "Carol" {
...
func TestDefaultStructurizer_LiftsSpeakerFromSupportingTurn(t *testing.T) {
	...
	f := model.TemporalFact{
		...
		EvidenceRefs: []model.EvidenceRef{{ID: "D1:3"}},
	}
	out := DefaultStructurizer{}.Structurize(f, Input{Turns: []TurnContext{turn}})
...
func TestDefaultStructurizer_ExtractsEntitiesFromContent(t *testing.T) {
	f := model.TemporalFact{Content: "Alice met Bob at the Eiffel Tower in Paris."}
	out := DefaultStructurizer{}.Structurize(f, Input{})
...
func TestDefaultStructurizer_FoldsKnownEntityAliases(t *testing.T) {
	...
	// surface variation.
	f := model.TemporalFact{Content: "Tom recommended a great espresso machine to Alice."}
	out := DefaultStructurizer{}.Structurize(f, Input{
...
func TestDefaultStructurizer_GrepsAbsoluteDateFromContent(t *testing.T) {
	f := model.TemporalFact{Content: "On 2024-05-07, Alice signed up for the gym."}
	out := DefaultStructurizer{}.Structurize(f, Input{})
...
func TestNopStructurizer_LeavesFactsUnchanged(t *testing.T) {
	f := model.TemporalFact{Content: "Alice loves Paris."}
