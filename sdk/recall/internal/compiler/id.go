...
// deterministic generators (e.g. counters) via Stages.IDGen.
type IDGenerator interface {
	NewID(f model.TemporalFact, now time.Time) string
}
...
func (g *timeOrderedGen) NewID(_ model.TemporalFact, now time.Time) string {
	g.mu.Lock()
	ms := now.UnixMilli()
	if ms <= g.last {
		ms = g.last
...
func (g *sequentialGen) NewID(model.TemporalFact, time.Time) string {
	g.mu.Lock()
	defer g.mu.Unlock()
	g.n++
