...
// This file ships three concrete port.Extractor implementations:
//
//   - passthroughExtractor: returns caller-supplied Facts verbatim.
//     The deterministic baseline used when Input.Turns is empty or
//     when callers explicitly construct structured facts.
//   - LLMExtractor: routes Input.Turns through a pluggable LLM
//     client that returns a JSON document matching
//     ExtractedFactSchema. The implementation uses sdk/llm and
//     tests exercise it with fake clients so no network calls are
//     required.
//   - StaticExtractor: returns a fixed list of facts, regardless
//     of input. Useful in tests that need deterministic non-empty
//     extraction without an LLM round trip.
//
// Implementations must respect Input.Scope: returned facts get
// their scope populated by the ingest core, but extractors should
// not invent scopes of their own.
...
// callers (a single port.TurnContext with just Text) still produce a
