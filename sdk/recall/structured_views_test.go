...
func TestRecall_StructuredQueryUsesRelationSource(t *testing.T) {
	...
	if !foundRelation {
		t.Errorf("relation source should have returned candidates, trace=%+v", trace.Sources)
	}
}
...
func TestRecall_TimelineProjectionRemovesSupersededStateOnSave(t *testing.T) {
	...
	for _, st := range trace.Sources {
		if st.Source == planner.SourceTimeline && st.Returned != 1 {
			t.Fatalf("timeline projection should evict superseded state before materialize, trace=%+v", trace.Sources)
		}
	}
	for _, drop := range trace.Drops {
		if drop.Source == planner.SourceTimeline && drop.Reason == DropSuperseded {
			t.Fatalf("timeline source should not emit superseded candidates after normal Save, drops=%+v", trace.Drops)
		}
	}
}
...
func TestRecall_ProfileBySubject(t *testing.T) {
	...
	if !found {
		t.Errorf("profile source missing from trace: %+v", trace.Sources)
	}
}
