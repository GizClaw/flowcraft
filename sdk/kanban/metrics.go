...
func newKanbanMetrics(ctx context.Context) *kanbanMetrics {
	...
	if err != nil {
		telemetry.Warn(ctx, "kanban: failed to create tasks.submitted metric", otellog.String("error", err.Error()))
	}
	...
	km.tasksDuration, err = m.Float64Histogram("kanban.tasks.duration.seconds",
		...
		metric.WithUnit("s"))
	if err != nil {
		telemetry.Warn(ctx, "kanban: failed to create tasks.duration metric", otellog.String("error", err.Error()))
	}

	km.agentsActive, err = m.Int64UpDownCounter("kanban.agents.active",
		metric.WithDescription("Number of currently active agents"))
	if err != nil {
		telemetry.Warn(ctx, "kanban: failed to create agents.active metric", otellog.String("error", err.Error()))
	}
