...
func (r *composeRecordingRunner) Exec(_ context.Context, _ string, _ []string, opts coresandbox.ExecOptions) (*coresandbox.ExecResult, error) {
	r.called = true
	r.opts = opts
...
func TestComposeLocal_ApprovalSeesEffectiveDefaults(t *testing.T) {
	inner := &composeRecordingRunner{}
	seenTimeout := time.Duration(0)
	runner := coresandbox.ComposeLocal(inner, coresandbox.LocalPolicy{
...
func TestDefaultLocalPolicy(t *testing.T) {
	root := t.TempDir()
	policy := coresandbox.DefaultLocalPolicy(root, nil, "rm")
