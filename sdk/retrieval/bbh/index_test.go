...
func TestSearchModesAndErrors(t *testing.T) {
	...
	if err := idx.Upsert(ctx, "ns", []retrieval.Doc{
		{ID: "c", Content: "gamma", Vector: []float32{2, 0}, Metadata: map[string]any{"kind": "rank"}, Timestamp: time.Unix(3, 0)},
		{ID: "d", Content: "delta", Vector: []float32{0, 1}, Metadata: map[string]any{"kind": "rank"}, Timestamp: time.Unix(4, 0)},
	}); err != nil {
		t.Fatal(err)
	}
	if err := idx.putDoc("ns", retrieval.Doc{
		ID:        "e",
		Content:   "epsilon",
		Vector:    []float32{1, 0, 0},
		Metadata:  map[string]any{"kind": "rank"},
		Timestamp: time.Unix(5, 0),
	}); err != nil {
		t.Fatal(err)
	}
	filteredRankedVec, err := idx.Search(ctx, "ns", retrieval.SearchRequest{
		QueryVector: []float32{1, 0},
		TopK:        2,
		Filter:      retrieval.Filter{Eq: map[string]any{"kind": "rank"}},
		MinScore:    -1,
	})
	if err != nil {
		t.Fatal(err)
	}
	if len(filteredRankedVec.Hits) != 2 ||
		filteredRankedVec.Hits[0].Doc.ID != "c" ||
		filteredRankedVec.Hits[1].Doc.ID != "d" {
		t.Fatalf("filtered ranked vector hits = %+v", filteredRankedVec.Hits)
	}
