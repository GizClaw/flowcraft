...
// the interfaces declared in sdk/agent — [agent.AfterExecute] and
// [agent.Hook] today, more if the agent package grows.
//
...
// # What lives here
//
//   - [AfterExecuteSuite] — the contract every [agent.AfterExecute]
//     implementation should pass.
//   - [HookSuite] — the contract every [agent.Hook]
//     implementation should pass.
//
// # What does NOT live here
//
// AfterExecute / Hook business logic — those are per-implementation
...
// AfterExecuteFactory builds a fresh [agent.AfterExecute] for each subtest.
// The suite invokes it once per case so subtests do not share
// decider state.
type AfterExecuteFactory func() agent.AfterExecute

// AfterExecuteCapabilities lets a decider opt out of subtests that
// don't apply. Most deciders pass the zero value (= every subtest
// runs).
type AfterExecuteCapabilities struct {
...
// AfterExecuteSuite runs every applicable contract subtest against
// deciders produced by f.
func AfterExecuteSuite(t *testing.T, f AfterExecuteFactory, caps ...AfterExecuteCapabilities) {
	t.Helper()
	c := AfterExecuteCapabilities{}
	if len(caps) > 0 {
		c = caps[0]
	}
...
func deciderZeroInput(t *testing.T, f AfterExecuteFactory) {
	t.Helper()
	d := f()
...
func deciderCtxCancel(t *testing.T, f AfterExecuteFactory) {
	t.Helper()
	d := f()
...
func deciderNoMutation(t *testing.T, f AfterExecuteFactory) {
	t.Helper()
	d := f()
...
func deciderAllStatuses(t *testing.T, f AfterExecuteFactory) {
	t.Helper()
...
func deciderConcurrent(t *testing.T, f AfterExecuteFactory) {
	t.Helper()
	d := f()
	const n = 16
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			...
			defer func() {
				if r := recover(); r != nil {
					t.Errorf("concurrent After panicked: %v — agent runtime invokes AfterExecute chains sequentially today, but stateless deciders should remain safe under concurrent reuse across runs", r)
				}
			}()
...
// offending method. Shared helper for both AfterExecuteSuite and
// HookSuite.
