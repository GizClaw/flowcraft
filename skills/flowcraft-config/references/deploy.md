...
| `inference.Provider` | `openai`, `anthropic`, `bytedance`, `minimax` | core contract; `openai` serves the whole OpenAI wire family (OpenAI, Azure, DeepSeek, Kimi, gateways) and `anthropic` the whole Messages family (Anthropic, MiniMax text, compatible endpoints) through their endpoint/wire/models settings |
