...
func TestCompositeHydratesThreeSourcesAndMissing(t *testing.T) {
	...
	records, err := messages.Append(ctx, messagesource.AppendRequest{
		...
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "message text")},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := chunks.ReplaceDocument(ctx, documentview.ReplaceRequest{
		...
		Chunks: []documentview.Chunk{{
			...
			DocumentVersion: 1, Content: textContent("chunk text"), Provenance: []corememory.SourceRef{source},
		}},
	}); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{component.CandidateAddress{Kind: corememory.ContextRawMessage, ConversationID: "conversation", ItemID: records[0].ID}, "message text"},
		{component.CandidateAddress{Kind: corememory.ContextFact, ConversationID: "conversation", ItemID: "fact-1"}, "fact text"},
		{component.CandidateAddress{Kind: corememory.ContextDocumentChunk, DatasetID: "dataset", DocumentID: "document", ItemID: "chunk-1"}, "chunk text"},
	}
	...
	missing := component.CandidateAddress{Kind: corememory.ContextFact, ConversationID: "conversation", ItemID: "missing"}
	...
	oldChunk := component.CandidateAddress{
		Kind: corememory.ContextDocumentChunk, DatasetID: "dataset", DocumentID: "document", ItemID: "chunk-1",
	}
...
func hydratedCandidate(address component.CandidateAddress, source corememory.SourceRef) component.Candidate {
