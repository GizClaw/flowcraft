...
const (
	// ModeWeighted is the calibrated weighted-sum fusion.
	ModeWeighted Mode = "weighted"
	// ModeRRF is reciprocal rank fusion: score = sum(weight / (K + rank)).
	// It is robust to score-scale differences between lanes and is the
	// leaderboard-aligned default.
	ModeRRF Mode = "rrf"
)

// Options configures the fusion algorithm.
type Options struct {
	// Mode selects the algorithm; the zero value selects RRF.
	Mode Mode
	// RRFK is the reciprocal-rank constant (default 60).
	RRFK float64
}
...
type Fusion struct {
	...
	mode  Mode
	rrfK  float64
	// ObserveLane, when set, receives each lane's wall time. Lanes run in
	// parallel, so this is what attributes a slow search across them.
	ObserveLane func(name string, elapsed time.Duration)
}

var _ component.Searcher = (*Fusion)(nil)

func New(lanes []Lane) (*Fusion, error) {
	return NewWithOptions(lanes, Options{Mode: ModeWeighted})
}

// NewWithOptions builds a fusion with an explicit algorithm.
func NewWithOptions(lanes []Lane, options Options) (*Fusion, error) {
	if len(lanes) == 0 {
	...
	if mode != ModeWeighted && mode != ModeRRF {
		return nil, fmt.Errorf("fusion: unknown mode %q", mode)
	}
	rrfK := options.RRFK
	if rrfK == 0 {
		rrfK = defaultRRFK
	...
	if math.IsNaN(rrfK) || math.IsInf(rrfK, 0) || rrfK < 0 {
		return nil, errors.New("fusion: rrf constant must be finite and non-negative")
	}
	...
	return &Fusion{lanes: owned, mode: mode, rrfK: rrfK}, nil
}
...
func (fusion *Fusion) SearchDetailed(ctx context.Context, request component.SearchRequest) (Result, error) {
	...
	if err := ctx.Err(); err != nil {
		return Result{}, corememory.NewError(corememory.KindOperationInterrupted, "context", err)
	}
	if err := request.Scope.Validate(); err != nil {
		return Result{}, corememory.NewError(corememory.KindInvalidRequest, "context", err)
	}
	if strings.TrimSpace(request.Query) == "" || request.Limit < 0 {
		return Result{}, corememory.NewError(
			corememory.KindInvalidRequest, "context", errors.New("fusion: query is required and limit must not be negative"),
		)
	}
	results := make([]laneSearchResult, len(fusion.lanes))
	completed := make(chan laneSearchResult, len(fusion.lanes))
	for i, lane := range fusion.lanes {
		go func(index int, current Lane) {
			started := time.Now()
			candidates, err := current.Searcher.Search(ctx, request)
			if fusion.ObserveLane != nil {
				fusion.ObserveLane(current.Name, time.Since(started))
			}
			completed <- laneSearchResult{index: index, candidates: candidates, err: err}
		}(i, lane)
	}
	for range fusion.lanes {
		select {
		case <-ctx.Done():
			return Result{}, corememory.NewError(corememory.KindOperationInterrupted, "context", ctx.Err())
	...
	if fusion.mode == ModeRRF {
		return fusion.fuseRRF(results, diagnostics, request), nil
	}
...
// fuseRRF merges lanes with reciprocal rank fusion: each successful lane
// contributes weight/(K + rank) for every candidate it returned, ranked by
// its native score. Scores are normalized so a candidate ranked first by
// every lane scores 1.
func (fusion *Fusion) fuseRRF(
	results []laneSearchResult,
	diagnostics []Diagnostic,
	...
	for i, result := range results {
		...
		best := make(map[string]component.Candidate, len(result.candidates))
		valid := true
		for _, candidate := range result.candidates {
			...
			key := stableKey(candidate)
			if prior, exists := best[key]; !exists || candidate.Score > prior.Score {
				best[key] = candidate
			}
		}
		if !valid {
		...
		sort.Slice(keys, func(left, right int) bool {
			if best[keys[left]].Score == best[keys[right]].Score {
				return keys[left] < keys[right]
			}
			return best[keys[left]].Score > best[keys[right]].Score
		})
		weight := fusion.lanes[i].Weight
		for rank, key := range keys {
			...
			contribution := weight / (fusion.rrfK + float64(rank+1))
			item.score += contribution
			item.candidate.Explanation.Terms = append(item.candidate.Explanation.Terms, component.ScoreTerm{
				Lane: fusion.lanes[i].Name, Raw: candidate.Score, Calibrated: contribution,
				Weight: weight, Contribution: contribution, CalibrationVersion: RRFAlgorithmVersion,
			})
		}
		totalWeight += weight
		contributed++
	}
	if contributed == 0 || totalWeight <= 0 {
		return Result{Candidates: []component.Candidate{}, Diagnostics: diagnostics}
	}
	maxPossible := totalWeight / (fusion.rrfK + 1)
	candidates := make([]component.Candidate, 0, len(merged))
	for _, item := range merged {
	...
	return Result{Candidates: candidates, Diagnostics: diagnostics}
}
