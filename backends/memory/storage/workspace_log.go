...
func (store *WorkspaceLog) ListStreams(ctx context.Context, prefix string) ([]string, error) {
	...
	err := workspace.Walk(ctx, store.ws, basePath, func(child string, entry fs.DirEntry) error {
		child = normalizeWalkPath(child)
		if !entry.IsDir() {
			return nil
		}
		if _, decodeErr := decodePathSegment(entry.Name()); decodeErr != nil {
			// Only adapter-encoded directories can hold streams; anything
			// else is foreign to this layout and is skipped.
