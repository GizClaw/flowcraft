...
func (store *MessageStore) Get(ctx context.Context, scope corememory.Scope, conversationID, messageID string) (Record, bool, error) {
...
func (store *MessageStore) List(ctx context.Context, scope corememory.Scope, conversationID string, options ListOptions) ([]Record, error) {
	if err := validateAddress(scope, conversationID); err != nil {
		return nil, err
	}
	stream, err := storage.StreamName(scope, conversationID)
...
func (store *MessageStore) Latest(ctx context.Context, scope corememory.Scope, conversationID string, options LatestOptions) ([]Record, error) {
	if err := validateAddress(scope, conversationID); err != nil {
		return nil, err
	}
	if options.Limit < 0 {
...
func (store *MessageStore) ListCommits(ctx context.Context, scope corememory.Scope, conversationID string, options ListCommitOptions) ([]Commit, error) {
	if err := validateAddress(scope, conversationID); err != nil {
		return nil, err
	}
	stream, err := storage.StreamName(scope, conversationID)
...
func (store *MessageStore) ListConversations(ctx context.Context, scope corememory.Scope) ([]string, error) {
...
func (store *MessageStore) commitByKey(ctx context.Context, scope corememory.Scope, conversationID, stream, key string) (Commit, error) {
	commitLog, ok := store.log.(storage.CommitLog)
	if !ok {
...
func (store *MessageStore) recordsFromEvents(scope corememory.Scope, conversationID string, events []storage.Event) ([]Record, error) {
...
func (store *MessageStore) recordFromEvent(scope corememory.Scope, conversationID string, event storage.Event) (Record, error) {
