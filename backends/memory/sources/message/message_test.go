...
var (
	messageScopeA = corememory.Scope{RuntimeID: "runtime", UserID: "alice", AgentID: "agent-a"}
	messageScopeB = corememory.Scope{RuntimeID: "runtime", UserID: "bob", AgentID: "agent-b"}
)
...
func TestMessageStoreAppendGetListAndRetry(t *testing.T) {
	...
	retry, err := store.Append(ctx, AppendRequest{
		Scope: messageScopeA, ConversationID: "conv", IdempotencyKey: "turn-1",
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "changed")},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMessageStoreCommitIdentityAndListingAreStable(t *testing.T) {
	...
	first, err := store.Commit(ctx, AppendRequest{
		Scope: messageScopeA, ConversationID: "conv", IdempotencyKey: "turn-1",
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "one")},
	})
	if err != nil {
		t.Fatal(err)
	}
	second, err := store.Commit(ctx, AppendRequest{
		...
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "two")},
	})
	if err != nil {
		t.Fatal(err)
	}
	retry, err := newMessageStore(t, ws).Commit(ctx, AppendRequest{
		Scope: messageScopeA, ConversationID: "conv", IdempotencyKey: "turn-1",
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "ignored")},
	})
...
func TestMessageStoreHardPartitionAndConversationIsolation(t *testing.T) {
	...
	for _, item := range []struct {
		...
		_, err := store.Append(ctx, AppendRequest{
			...
			Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, item.text)},
		})
		if err != nil {
			t.Fatal(err)
		}
...
func TestMessageStorePersistsAndPaginatesStably(t *testing.T) {
	...
	for index := 0; index < 5; index++ {
		...
		_, err := store.Append(ctx, AppendRequest{
			...
			Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, fmt.Sprint(index))},
		})
		if err != nil {
			t.Fatal(err)
		}
...
func TestMessageStoreCommitOrderComesFromSeqNotIdempotencyKey(t *testing.T) {
	...
	for _, turn := range []struct{ key, text string }{
		...
		if _, err := store.Append(ctx, AppendRequest{
			...
			Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, turn.text)},
		}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestMessageStoreOwnsInputsAndResults(t *testing.T) {
	ctx := context.Background()
	store := newMessageStore(t, newTestWorkspace(t))
	input := coremessage.NewTextMessage(coremessage.RoleUser, "original")
	...
	result, err := store.Append(ctx, AppendRequest{
		...
		Messages: []coremessage.Message{input}, Metadata: metadata,
	})
	if err != nil {
		t.Fatal(err)
	}
	input.Content.Parts[0] = coremessage.TextPart{Text: "input mutation"}
	metadata["key"] = "input mutation"
	result[0].Message.Content.Parts[0] = coremessage.TextPart{Text: "result mutation"}
	...
	got.Message.Content.Parts[0] = coremessage.TextPart{Text: "get mutation"}
...
func TestMessageStoreEncodesMaliciousIDs(t *testing.T) {
	...
	records, err := store.Append(ctx, AppendRequest{
		...
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "safe")},
	})
...
func TestMessageStoreValidationAndNilDependencies(t *testing.T) {
	...
	if _, err := store.Append(context.Background(), AppendRequest{
		...
		Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, "x")},
	}); err == nil {
...
func TestMessageStoreConversationOrderingIsSorted(t *testing.T) {
	...
	for _, conversation := range input {
		_, err := store.Append(ctx, AppendRequest{
			...
			Messages: []coremessage.Message{coremessage.NewTextMessage(coremessage.RoleUser, conversation)},
		})
		if err != nil {
			t.Fatal(err)
		}
