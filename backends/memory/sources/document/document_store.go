...
func (store *DocumentStore) Put(ctx context.Context, request PutRequest) (Document, error) {
	...
	if hasCurrent {
		...
		if current.Event.Document != nil {
			...
			// The document was tombstoned: preserve its original creation
			// time. The first-write marker answers this in one read; only
			// documents created before the marker existed fall back to the
			// scope-wide event scan.
			recorded, found, err := store.readFirstWrite(ctx, request.Scope, request.DatasetID, request.DocumentID)
			if err != nil {
				return Document{}, err
			}
			if found {
	...
	} else if recorded, found, err := store.readFirstWrite(ctx, request.Scope, request.DatasetID, request.DocumentID); err != nil {
		return Document{}, err
	} else if found {
		// The current pointer is gone but the document existed before;
		// keep its original creation time.
		createdAt = recorded
	}
	document := Document{
		Scope:      request.Scope,
	...
	event := Event{
		...
		Provenance: append([]corememory.SourceRef(nil), provenance...), CreatedAt: now,
	}
	...
	// appendEvent resolves an existing immutable address (same version and
	// operation) to the prior event. That is only correct when the caller
	// retried identical content; a different payload must surface as a
	// conflict instead of silently discarding the write.
	if authoritative.Event.Document == nil {
		return Document{}, errdefs.Conflictf(
			"document source: document %q version %d conflicts with a different payload",
			request.DocumentID, document.Version)
	}
	unchanged, err := sameDocumentPayload(*authoritative.Event.Document, document)
	if err != nil {
		return Document{}, err
	}
	if !unchanged {
	...
	if err := store.ensureFirstWrite(ctx, request.Scope, request.DatasetID, request.DocumentID, document.CreatedAt); err != nil {
		return Document{}, err
	}
...
func (store *DocumentStore) Get(ctx context.Context, scope corememory.Scope, datasetID, documentID string) (Document, bool, error) {
...
func (store *DocumentStore) List(ctx context.Context, scope corememory.Scope, datasetID string, options ListOptions) ([]Document, error) {
	if err := validateAddress(scope, datasetID); err != nil {
		return nil, err
	}
	store.mu.RLock()
...
func (store *DocumentStore) ListDatasets(ctx context.Context, scope corememory.Scope) ([]string, error) {
...
func (store *DocumentStore) ListEvents(ctx context.Context, scope corememory.Scope, options ListEventOptions) ([]Event, error) {
	if err := scope.Validate(); err != nil {
		return nil, err
	}
	store.mu.RLock()
...
// treated as success.
func (store *DocumentStore) Delete(ctx context.Context, scope corememory.Scope, datasetID, documentID string) error {
	if err := validateAddress(scope, datasetID); err != nil {
		return err
	}
	if strings.TrimSpace(documentID) == "" {
...
func (store *DocumentStore) DeleteDataset(ctx context.Context, scope corememory.Scope, datasetID string) error {
	if err := validateAddress(scope, datasetID); err != nil {
		return err
	}
	store.mu.Lock()
...
func (store *DocumentStore) tombstone(ctx context.Context, scope corememory.Scope, datasetID, documentID string) error {
	...
	event := Event{
		...
		Provenance: append([]corememory.SourceRef(nil), current.Event.Provenance...),
...
func (store *DocumentStore) scanDataset(ctx context.Context, scope corememory.Scope, datasetID string) ([]Document, error) {
	prefix, err := store.currentDatasetPrefix(scope, datasetID)
	if err != nil {
		return nil, err
	}
...
func (store *DocumentStore) documentEvents(ctx context.Context, scope corememory.Scope, datasetID, documentID string) ([]persistedEvent, error) {
	stream, err := store.eventsStream(scope)
	if err != nil {
		return nil, err
	}
...
func (store *DocumentStore) readByKey(ctx context.Context, scope corememory.Scope, datasetID, documentID, key string) (persistedEvent, bool, error) {
...
func (store *DocumentStore) readCurrent(ctx context.Context, scope corememory.Scope, datasetID, documentID string) (persistedEvent, bool, error) {
...
func (store *DocumentStore) scopePrefix(scope corememory.Scope) (string, error) {
...
func (store *DocumentStore) eventsStream(scope corememory.Scope) (string, error) {
...
func (store *DocumentStore) currentPrefix(scope corememory.Scope) (string, error) {
	prefix, err := store.scopePrefix(scope)
	if err != nil {
		return "", err
	}
...
func (store *DocumentStore) currentDatasetPrefix(scope corememory.Scope, datasetID string) (string, error) {
	prefix, err := store.currentPrefix(scope)
	if err != nil {
		return "", err
	}
...
func (store *DocumentStore) currentKey(scope corememory.Scope, datasetID, documentID string) (string, error) {
	prefix, err := store.currentDatasetPrefix(scope, datasetID)
	if err != nil {
		return "", err
	}
...
// firstWrite is the immutable creation record of one document. It survives
// tombstoning so a re-created document keeps its original creation time
// without scanning the scope-wide event log.
type firstWrite struct {
	SchemaVersion int       `json:"schema_version"`
	CreatedAt     time.Time `json:"created_at"`
}

func (store *DocumentStore) firstWriteKey(scope corememory.Scope, datasetID, documentID string) (string, error) {
	prefix, err := store.scopePrefix(scope)
	if err != nil {
		return "", err
	}
	return prefix + "/first/" + storage.EncodeSegment(datasetID) + "/" + storage.EncodeSegment(documentID), nil
}

func (store *DocumentStore) readFirstWrite(
	ctx context.Context,
	scope corememory.Scope,
	datasetID, documentID string,
) (time.Time, bool, error) {
	key, err := store.firstWriteKey(scope, datasetID, documentID)
	if err != nil {
	...
	if err != nil {
		...
		return time.Time{}, false, fmt.Errorf("document source: read first write %q: %w", documentID, err)
	}
	var record firstWrite
	if err := decodeJSON(data, &record); err != nil {
		return time.Time{}, false, fmt.Errorf("document source: decode first write %q: %w", documentID, err)
	}
	if record.SchemaVersion != documentSchemaVersion || record.CreatedAt.IsZero() {
		return time.Time{}, false, fmt.Errorf("document source: corrupt first write %q", documentID)
	}
	return record.CreatedAt.UTC(), true, nil
}

func (store *DocumentStore) ensureFirstWrite(
	ctx context.Context,
	scope corememory.Scope,
	datasetID, documentID string,
	createdAt time.Time,
	...
	payload, err := json.Marshal(firstWrite{SchemaVersion: documentSchemaVersion, CreatedAt: createdAt.UTC()})
	if err != nil {
		return err
	}
	put, ok := store.kv.(storage.PutIfAbsentStore)
	...
	if !bytes.Equal(existing, payload) {
		return errdefs.Conflictf("document source: first write record for %q conflicts", documentID)
	}
	return nil
}

// sameDocumentPayload reports whether two revisions carry the same
// caller-supplied payload. Authoritative timestamps are excluded so a retry
// with a fresh clock is not mistaken for a different document.
func sameDocumentPayload(left, right Document) (bool, error) {
	leftContent, err := json.Marshal(left.Content)
	if err != nil {
		return false, fmt.Errorf("document source: encode content: %w", err)
	}
	...
	if !slices.Equal(left.Provenance, right.Provenance) {
		return false, nil
	}
	return maps.Equal(left.Metadata, right.Metadata), nil
}

func (store *DocumentStore) byKeyKey(scope corememory.Scope, datasetID, documentID, key string) (string, error) {
	prefix, err := store.scopePrefix(scope)
	if err != nil {
		return "", err
	}
...
func validatePersistedEvent(persisted persistedEvent, scope corememory.Scope, datasetID, documentID, key string) error {
...
func validateDocument(document Document, scope corememory.Scope, datasetID, documentID string) error {
...
func validateAddress(scope corememory.Scope, datasetID string) error {
	if err := scope.Validate(); err != nil {
		return err
	}
