...
var (
	documentScopeA = corememory.Scope{RuntimeID: "runtime", UserID: "alice", AgentID: "agent-a"}
	documentScopeB = corememory.Scope{RuntimeID: "runtime", UserID: "bob", AgentID: "agent-b"}
	testSource     = corememory.SourceRef{Kind: corememory.SourceExternal, ID: "source-1", Locator: "file:///source"}
)

func TestDocumentStorePutReplaceAndRetry(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 8, 4, 12, 0, 0, 0, time.UTC)
	store := newDocumentStore(t, newTestWorkspace(t), WithClock(func() time.Time { return now }))
...
// TestDocumentStoreDifferentPayloadConflict guards against silently
// discarding a write: when the immutable address already exists with other
// content (a crashed predecessor write), the second write must fail with a
// conflict instead of returning the stored document.
func TestDocumentStoreDifferentPayloadConflict(t *testing.T) {
	ctx := context.Background()
	...
	if _, err := store.Put(ctx, documentRequest(documentScopeA, "dataset", "doc", "put-1", "durable")); err == nil {
	...
	// A different idempotency key and a different payload now target the
	// same immutable address (version 1, put) that the crashed write left
	// behind.
	_, err = reopened.Put(ctx, documentRequest(documentScopeA, "dataset", "doc", "put-2", "different"))
	if !errdefs.IsConflict(err) {
		t.Fatalf("overwrite error = %v, want conflict", err)
	}
	current, err := reopened.List(ctx, documentScopeA, "dataset", ListOptions{})
	if err != nil {
		t.Fatal(err)
	}
	if len(current) != 0 {
		t.Fatalf("conflicting write advanced current = %#v", current)
	}
}

// TestDocumentStoreRecreateAfterDeleteKeepsCreatedAt pins the first-write
// marker: a document re-created after a tombstone keeps its original
// creation time without re-reading the scope-wide event log.
func TestDocumentStoreRecreateAfterDeleteKeepsCreatedAt(t *testing.T) {
	ctx := context.Background()
	now := time.Date(2026, 8, 4, 12, 0, 0, 0, time.UTC)
	kvStore, err := storage.NewWorkspaceKV(newTestWorkspace(t))
	if err != nil {
	...
	counted := &countingLog{Log: logStore}
	store, err := NewDocumentStore(counted, kvStore, WithClock(func() time.Time { return now }))
	if err != nil {
		t.Fatal(err)
	}
	first, err := store.Put(ctx, documentRequest(documentScopeA, "dataset", "doc", "put-1", "first"))
	...
	if err := store.Delete(ctx, documentScopeA, "dataset", "doc"); err != nil {
		t.Fatal(err)
	}
	now = now.Add(24 * time.Hour)
	counted.reset()
	recreated, err := store.Put(ctx, documentRequest(documentScopeA, "dataset", "doc", "put-2", "second"))
	if err != nil {
		t.Fatal(err)
	}
	if !recreated.CreatedAt.Equal(first.CreatedAt) {
		t.Fatalf("created_at = %v, want %v", recreated.CreatedAt, first.CreatedAt)
	}
	if !recreated.UpdatedAt.Equal(now) {
		t.Fatalf("updated_at = %v, want %v", recreated.UpdatedAt, now)
	}
	if reads := counted.reads(); reads != 0 {
		t.Fatalf("event log reads = %d, want 0 with the first-write marker", reads)
	}
}
...
func TestDocumentStoreHardPartitionAndDatasetIsolation(t *testing.T) {
	ctx := context.Background()
	store := newDocumentStore(t, newTestWorkspace(t))
	requests := []PutRequest{
...
func TestDocumentStoreAgentPartitionIsolation(t *testing.T) {
	ctx := context.Background()
	store := newDocumentStore(t, newTestWorkspace(t))
	scopeA := corememory.Scope{RuntimeID: "runtime", UserID: "same-user", AgentID: "agent-a"}
	scopeB := corememory.Scope{RuntimeID: "runtime", UserID: "same-user", AgentID: "agent-b"}
	global := corememory.Scope{RuntimeID: "runtime", UserID: "same-user"}
...
func TestDocumentStoreOwnsInputsAndResults(t *testing.T) {
	...
	content := coremessage.Content{Parts: []coremessage.Part{coremessage.DataPart{Value: raw}}}
	provenance := []corememory.SourceRef{testSource}
	metadata := corememory.Metadata{"key": "original"}
	request := PutRequest{
	...
	part := put.Content.Parts[0].(coremessage.DataPart)
	part.Value[10] = 'Y'
	...
	gotPart := got.Content.Parts[0].(coremessage.DataPart)
	...
	if string(again.Content.Parts[0].(coremessage.DataPart).Value) != `{"value":"original"}` ||
...
func documentRequest(scope corememory.Scope, dataset, documentID, key, text string) PutRequest {
	return PutRequest{
		...
		Provenance: []corememory.SourceRef{testSource},
		Metadata:   corememory.Metadata{"text": text},
	}
}
...
func mustDocumentStore() *DocumentStore {
	logStore, err := storage.NewWorkspaceLog(mustWorkspace())
	if err != nil {
		panic(err)
	}
	kvStore, err := storage.NewWorkspaceKV(mustWorkspace())
	if err != nil {
		panic(err)
	}
	store, err := NewDocumentStore(logStore, kvStore)
...
func mustWorkspace() workspace.Workspace {
	dir, err := os.MkdirTemp("", "flowcraft-memory-document-")
	if err != nil {
		panic(err)
	}
...
// countingLog counts Read calls so a test can prove a code path avoids a
...
func (log *countingLog) Read(ctx context.Context, stream string, after uint64, limit int) ([]storage.Event, error) {
	log.mu.Lock()
	log.readCount++
	log.mu.Unlock()
	return log.Log.Read(ctx, stream, after, limit)
}

func (log *countingLog) reset() {
	log.mu.Lock()
	log.readCount = 0
	log.mu.Unlock()
}

func (log *countingLog) reads() int {
	log.mu.Lock()
	defer log.mu.Unlock()
	return log.readCount
}
