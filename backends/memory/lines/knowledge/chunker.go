...
const (
	...
	// MaxChunkRunes bounds one derived chunk. Chunks are model-visible
	// retrieval items, so the hard ceiling keeps a single item below the
	// context provider's 10k-token item limit even for CJK text.
	MaxChunkRunes = 8_000
	// MaxSummaryRunes bounds one deterministic hierarchy summary.
...
func NewChunker(config ChunkerConfig) (*Chunker, error) {
	...
	if config.MaxRunes > MaxChunkRunes {
		return nil, fmt.Errorf("knowledge line: max_runes must not exceed %d", MaxChunkRunes)
	}
	...
	if config.Summary.MaxRunes > MaxSummaryRunes {
		return nil, fmt.Errorf("knowledge line: summary max_runes must not exceed %d", MaxSummaryRunes)
	}
...
func (chunker *Chunker) Derive(ctx context.Context, input component.Artifact) ([]component.Artifact, error) {
	...
	identitySources := make([]corememory.SourceRef, 0, len(sources))
	for _, source := range sources {
		if source.Kind == corememory.SourceDocument {
			source.Locator = ""
	...
	output := []component.Artifact{{
		...
		Sources: append([]corememory.SourceRef(nil), sources...), Metadata: resourceMetadata,
	}}
	if chunker.summary.Document {
		...
		output = append(output, component.Artifact{
			...
			Sources: append([]corememory.SourceRef(nil), sources...),
...
func textContent(value string) coremessage.Content {
	return coremessage.Content{Parts: []coremessage.Part{coremessage.TextPart{Text: value}}}
}

func hierarchyMetadata(base corememory.Metadata, kind string, level int, parentID string, ordinal int,
...
func hierarchyID(sources []corememory.SourceRef, kind, position, content string) (string, error) {
	payload := struct {
		Sources  []corememory.SourceRef `json:"sources"`
...
func identityDigest(sources []corememory.SourceRef) (string, error) {
