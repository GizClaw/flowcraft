...
func validateActiveHead(head activeCatalogHead, scope corememory.Scope, conversationID string) error {
...
func validateActiveBase(base activeCatalogBase, scope corememory.Scope, conversationID string, head activeCatalogHead) error {
	digest := base.Digest
	base.Digest = ""
...
func (store *SummaryStore) catalogKey(scope corememory.Scope, conversationID string) string {
	return strings.Join([]string{scope.RuntimeID, scope.UserID, scope.AgentID, conversationID}, "\x00")
}

func (store *SummaryStore) activeBasePath(scope corememory.Scope, conversationID, id string) (string, error) {
...
func (store *SummaryStore) activeSegmentPath(scope corememory.Scope, conversationID, id string) (string, error) {
	root, err := store.activeCatalogRoot(scope, conversationID)
	if err != nil {
		return "", err
	}
...
func (store *SummaryStore) activeCatalogRoot(scope corememory.Scope, conversationID string) (string, error) {
