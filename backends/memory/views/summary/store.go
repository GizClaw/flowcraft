...
func (store *SummaryStore) List(ctx context.Context, scope corememory.Scope, conversationID string, options ListOptions) ([]Record, error) {
	if err := validateAddress(scope, conversationID); err != nil {
		return nil, err
	}
	if options.Level != nil {
...
func (store *SummaryStore) listLocked(ctx context.Context, scope corememory.Scope, conversationID string, options ListOptions) ([]Record, error) {
...
func (store *SummaryStore) read(ctx context.Context, scope corememory.Scope, conversationID, id string) (Record, bool, error) {
	...
	if value.RuntimeID != scope.RuntimeID || value.UserID != scope.UserID || value.AgentID != scope.AgentID ||
		...
		return Record{}, false, fmt.Errorf(
			"summary view: persisted address %q does not match record key %q (key %q)",
			value.RecordID, id, key)
	}
...
func (store *SummaryStore) recordsDir(scope corememory.Scope, conversationID string) (string, error) {
...
func (store *SummaryStore) recordsStream(scope corememory.Scope, conversationID string) (string, error) {
...
func (store *SummaryStore) recordPath(scope corememory.Scope, conversationID, id string) (string, error) {
	dir, err := store.recordsDir(scope, conversationID)
	if err != nil {
		return "", err
	}
...
func (store *SummaryStore) manifestPath(scope corememory.Scope, conversationID string) (string, error) {
	partition, err := storage.ScopePartition(scope)
	if err != nil {
		return "", err
	}
