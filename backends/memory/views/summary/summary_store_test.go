...
var summaryScope = corememory.Scope{RuntimeID: "runtime", UserID: "user"}
...
func TestSearcherReturnsOnlyActiveManifestRecords(t *testing.T) {
	...
	got, err := searcher.Search(ctx, component.SearchRequest{
		Scope: summaryScope, Query: "", Metadata: corememory.Metadata{"conversation_id": "conversation"},
	})
	...
	oldGeneration, err := searcher.Search(ctx, component.SearchRequest{
		...
		Metadata: corememory.Metadata{"conversation_id": "conversation", "generation_id": "generation-1"},
	})
...
func summaryRequest(id string) AddRequest {
	return AddRequest{
		...
		SourceRefs: []corememory.SourceRef{{
			Kind: corememory.SourceMessage, ID: "conversation/message-1", Revision: "1",
		}},
