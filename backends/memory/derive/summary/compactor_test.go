...
func TestCompactorBuildsFourLevelsRespectsThresholdDepthAndReplay(t *testing.T) {
	...
	logStore, _ := storage.NewWorkspaceLog(newTestWorkspace(t))
	kvStore, _ := storage.NewWorkspaceKV(newTestWorkspace(t))
...
// pass the complete input window: the manifest replaces the previous one, so
// records derived from omitted inputs disappear from the active catalog.
func TestCompactorPartialWindowReplacesTheCatalog(t *testing.T) {
	ctx := context.Background()
	compactor, _ := New(DefaultConfig(), mustStore(t), echoSummarizer{})
	if _, err := compactor.Compact(ctx, CompactRequest{
		Scope: testScope(), ConversationID: "conversation", GenerationID: "generation-3", Inputs: inputs(3),
	}); err != nil {
		t.Fatal(err)
	}
	active, err := compactor.Compact(ctx, CompactRequest{
		Scope: testScope(), ConversationID: "conversation", GenerationID: "generation-partial",
		Inputs: inputs(3)[1:],
	})
	if err != nil {
		t.Fatal(err)
	...
	for _, record := range active {
		for _, id := range record.InputIDs {
			if id == "fact-00" {
				t.Fatalf("omitted input survived the replacement manifest: %#v", active)
			}
		}
	}
}
...
func inputs(count int) []Input {
	...
	for index := range result {
		result[index] = Input{
			...
			Topics: []string{"topic"}, SourceRefs: []corememory.SourceRef{{
				Kind: corememory.SourceMessage, ID: fmt.Sprintf("conversation/message-%02d", index),
