...
//
// The journal watches host-side paths, which is exactly what bwrap
// exposes to the sandbox: the root and the writable paths are
// bind-mounted at their own absolute paths, so a write inside the
// namespace is a write to the same host path. Events under the root are
...
// confinement bwrap enforces decides what is allowed, and the journal
