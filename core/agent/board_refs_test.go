...
func TestResolveStringEscaped(t *testing.T) {
	...
	// The unified escape rule is "backslash immediately before \${":
	// an even run leaves one literal backslash plus the literal
	// reference.
	got, err = board.ResolveString(`\\${board:x}`)
	if err != nil || got != `\${board:x}` {
		t.Fatalf("even backslashes = %v, %v; want \\${board:x}", got, err)
	}
}
